...
log = logging.getLogger('awmenu.common.utils')
...
def get_deployment_mode():
    ...
    try:
        result = os.path.isdir('/var/lib/aw/keys/master')
        if result:
...
def get_version(versionfile=consts.RELEASE_FILE):
    """Read version from versionfile or return empty string."""
    try:
        with open(versionfile, 'r') as f:
            return f.read().strip()
    except IOError:
        log.error("Unable to set version from %s" % versionfile)
        return ""
...
def lock_running(lock_file):
    ...
    try:
        ...
        print("Another copy of awmenu is running. "
