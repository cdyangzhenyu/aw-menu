...
from awmenu.common import dialog
from awmenu.common import network
from awmenu.common import timeout
from awmenu.common import urwidwrapper as widget
from awmenu.common import utils
from awmenu import consts
from awmenu import modules
from awmenu import settings as settings_module
...
log = logging.getLogger('awmenu.loader')


class AwSetup(object):
    ...
    def __init__(self, save_only=False, managed_iface=None):
        ...
        self.settings.json_load(consts.SETTINGS_FILE)
    ...
    def main(self):
        text_header = consts.AWCLOUD_TITLE
        text_footer = consts.AWCLOUD_FOOTER
        ...
        self.menuitems = self.menu(consts.AWCLOUD_MENU, self.visiblechoices)
    ...
    def global_save(self):
        ...
        self.settings.json_write(outfn=consts.SETTINGS_FILE)
    ...
    def global_task(self, ):
        #1. systemctl restart auto-register.
        #2. start saas vm or docker.
        #3. exec ansible code.
        pass
    ...
    def _save_only(self):
        if utils.is_post_deployment():
            print(
                ...
                ".\nRun awmenu manually to make changes."
            )
            sys.exit(0)
...
def setup(**kwargs):
    urwid.web_display.set_preferences("Aw Setup")
...
def main(*args, **kwargs):
    ...
    if options.save_only:
        ...
        if not os.isatty(sys.stdin.fileno()):
            print("Stdin is not a tty, can't run awmenu "
                  "in interactive mode.")
            sys.exit(1)
        setup()
